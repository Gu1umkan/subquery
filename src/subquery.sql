...
create type genres as enum ( 'DETECTIVE','DRAMA','HISTORY','ROMANCE','BIOGRAPHY','FANTASY');
...
select *
from books
where published_year between '2020-01-01' and '2023-12-31';
...
select concat(first_name, ' ', last_name) as author, country
from authors
...
-- 13. Роман жана Детектив китептеринин эн арзан бааларын чыгарыныз
    select min(price) as mi_price , genre from books  where genre in('ROMANCE','DETECTIVE') group by genre;
...
-- 15.Китептердин , издательстволордун аттары жана тили чыксын
    select  b.name as book_name,p.name as publisher_name, l.language from books b
...
-- 16.Авторлордун бардык маалыматтары жана издательстволору чыксын, издательство болбосо null чыксын
    select a.*, p.name as publisher_name from authors a left join books b on a.id = b.author_id left join publishers p on  p.id = b.publisher_id;

-- 17.Авторлордун толук аты-жону жана китептери чыксын, китеби жок болсо null чыксын.
select a.first_name,a.last_name, b.name as book_name from authors a left join books b on a.id = b.author_id;

-- 18.Кайсы тилде канча китеп бар экендиги ылдыйдан ойлдого сорттолуп чыксын.

select l.language, language_id, COUNT(*) as book_count
from books b
inner join languages l on  b.language_id = l.id
group by  b.language_id, l.language
...
select a.first_name, a.last_name, sum(b.price) from authors a
